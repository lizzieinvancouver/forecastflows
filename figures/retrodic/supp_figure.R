...

{
  ...
  spid <- c()
  for(i in 1:nspecies){
    nperspecies <- ifelse(i %in% c(1:3), 20, 3)
  ...
  minlim <- min(min(residnophylo$X50.), min(residphylo$X50.))
  maxlim <- max(max(residnophylo$X50.), max(residphylo$X50.))
  # maxlim <- 1 
  
  resid <- residnophylo$X50.
  names(resid) <- tree$tip.label
  fit.fastAnc <- phytools::fastAnc(tree, resid, vars = TRUE, CI = TRUE)
  td <- data.frame(
    node = ggtree::nodeid(tree, names(resid)),
    trait = resid)
  nd <- data.frame(node = names(fit.fastAnc$ace), trait = fit.fastAnc$ace)
  d <- rbind(td, nd)
  ...
  plottree_nophylo <- ggtree::ggtree(
    ...
    scale_colour_viridis_c(limits = c(minlim,maxlim)) + 
    theme(
  ...
  summ_fitnophylo <- data.frame(summary(fit)[["summary"]])
  summ_fitphylo <- data.frame(summary(fitphylo)[["summary"]])
  
  slopesnophylo <- data.frame(
    slp_fit = summ_fitnophylo[paste0("mu_beta_sp[",1:nspecies,"]"), c('X50.')],
    slp_fit.lcl = summ_fitnophylo[paste0("mu_beta_sp[",1:nspecies,"]"), c('X2.5.')],
    slp_fit.ucl = summ_fitnophylo[paste0("mu_beta_sp[",1:nspecies,"]"), c('X97.5.')],
    sp = 1:nspecies,
    mod = 'Standard'
  )
  ...
  plot_slopes <- ggplot(data = rbind(slopesnophylo, slopesphylo)) +
    # geom_vline(aes(xintercept = 0), linetype = 'dashed', color = 'grey40') +
    geom_pointrange(aes(y = sp, xmin = slp_fit.lcl, xmax = slp_fit.ucl,x = slp_fit, group = mod, color = mod),
                    position = position_dodge(width = 0.5), size = 0.3) +
    theme_classic() +
    theme(axis.line.y = element_blank(), axis.ticks.y = element_blank(),
          axis.text.y = element_blank(), axis.title.y = element_blank(),
  ...
  plottree_nophylo + plot_slopes + plottree_phylo
}
