...
\begin{figure}[b]
	...
	Interestingly, when looking at the interquartile range of these residuals (middle part of the figure in cyan and blue, with lines representing IQR and dots the medians), both models (with and without phylogenetic structure) seem less distinct. This illustrates that we may want to consider building a more adapted retrodictive check. The addition of correlation structure in our second model should indeed mostly result in a reduction in species-specific parameter uncertainty rather than an important shift to the $y_\text{pred}$ themselves. One might need to develop a smarter check, for example looking at empirical covariance between species two by two.} %emwJul24 -- not sure about last two sentences, could be hard for anyone but us to follow? 
	\label{fig:modeldata}
	\vspace*{2cm}
\end{figure}
