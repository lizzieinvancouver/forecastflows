...
%emw24Jul: I won't flag all of them, but check for dot dot dot and do a good spellcheck before submitting. 
...
% here I hope we will have a PPC figure
...
%emw24Jul:  I never use sorry in these letters, it sounds less formal in a bad way to me, but that may be just me. 
...
%emw24Jul: watch out of too many nominalizations (eg conceptualization) 
...
%emw24Jul: If you add the retro check I think this would be a good example, you could also reference the workflow again? In fact here and elsewhere perhaps I think we could say in response: Great point, we realized we could also better reference the example workflow we provide as supplemental files, which we now do XX times in the paper. 
...
% Jim: I think the reviewer makes a fair point that this is vague. Can we add something here, or even just refer to section 3.2 where (I think) relevant examples are already given in the final paragraph?
...
... %emw24Jul: Say something here like: Great, as mentioned above we have tried to add more pointers to what we mean and how to apply this workflow through the revised manuscript. 
...
% Hmm, not sure? "For examples using LPI data" and then we talk about several examples right? %emw24Jul: No, 'For example' is the only way we ever say this in English. It's just a phrase. 
...
%emw24Jul: See what you think of my edits! Which I made before reading this (wow). 
...
Thank you for this suggestion. We added a paragraph in the introduction to discuss the inferential goals of these two approachs (\href{file:forecastflows_r1\#lintarget:inferentialgoal}{line \ref*{lin:inferentialgoal}}). We also changed the title to reflect the broader inferential goals. %emw24Jul: do we mean we changed section title? If so, you can ignore my comments in the other file on the paper's title!
...
This is a good point and we could have been clearer on different frameworks that the workflow can be used with. We have worked to clarify that in several places in this revision. %emw24Jul: Add line numbers? I think there are two places we now say this.... %emw24Jul: try to avoid saying 'not in scope' and 'no space' when possible. 
...
% Real-data examples... not do-able!
... %emw24Jul: No but this would be a great place be sure to reference the worked example and to say to the reviewer: Good point. We provide a worked example, but we realized upon receiving this feedback, that we could better reference the example workflow here and in other places, which we now do. 
...
...
We now mention that the summary statistics should be developed specifically for the research question (\href{file:forecastflows_r1\#lintarget:howtoPPC}{line \ref*{lin:howtoPPC}}). % I already said this above...
