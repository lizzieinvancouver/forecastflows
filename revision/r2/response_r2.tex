...
Editor and reviewer comments are in \emph{gray italics} (we provide below the full context of the reviewers 1-2 comments below, and extract the parts of reviewer 3's comments that required changes to this manuscript), while our responses are in regular text. \\ 
...
We are glad that the reviewer found the manuscript has improved, and thank them for all their feedback to improve the manuscript. 
...
% emw18Oct: It sounds weird and the reviewer does not like it so I changed it.
Corrected.
...
We appreciate this point, however, we wanted our workflow to be approachable. We specifically wanted to leave room for readers to think through how they would advance upon it (to whatever degree of complexity they see as biological relevant), thus, we have left it as is.  
...
We now specify that the first posterior predictive distribution we show corresponds to the predicted counts across all species and populations, and show nine (9) draws.
...
% emw18Oct: I don't think so, I commented it out. 
...
This is a good point, we have clarified our use of this word in the two areas we used it without additional context (lines, \href{file:forecastflows_r2\#lintarget:bias1}{\ref*{lin:bias1}}, \href{file:forecastflows_r2\#lintarget:bias2}{\ref*{lin:bias2}}). % emw18Oct: fix these refs to main text!
...
We would like to thank the reviewer once again for all these very interesting avenues for discussion---and look forward to discussions with him directly. % beyond the community of researchers (to whom this paper was primarily addressed).
