...
Our first goal with simulated data is a simple check of our code (both the model code and simulated data code), given a high enough sample size relative to our effect sizes and error, the model should return our paramaters. So our first step is to check this. 
...
Now we use our fitted model parameters to generate predictions, and compare them to observations. This is similar to the simulating data above, but differs in two major ways: (1) we simulate from the paramaters estimated by the model fit to the empirical data (versus coming up with them based on our expert knowledge) and (2) we need to check across many predictions from the model to understand the full uncertainty of the model (and not just one draw from the model). 

Here we start by examining how well predictions from the mean estimated values from the model compare to the observed data; this is similar to many plots where we simply want to see how well the model seems to predict the data. 

To do this, we start by extracting the model parameter estimates:
...
```
ADD HERE (and adjust code above): Something about what the overall trends appears to be, but that to see if that might be a good estimate we need to check how the model predicts the data on finer scales. So we start by examining some species separately, e.g. here the African bush elephant:
...
Focusing on populations in South Africa, we see some places where the linear model apears well (ID 7673), but also two other populations where it does not appear as a very good fit. 
...
The three populations are measured in the same units, but are very different in their sizses (ranging from a few hundred individuals to more than 10000), thus they likely represent populations measured across different spatial scales.
...
At this point, we would have a lot of ideas of ways to improve this model that we would want to follow up on, including (but not limited to) questioning our assumption of one sigma for all populations (CHECK HERE: do we really use one sigma for all populations?) and potentially our linearity assumption. One possibility would be to move to a non-linear regression, and use a more flexible model that would allow population numbers to stabilize, such as shown using a Gompertz model for the wildbeest dataset in Tanzania:
...
We also have seen variability in population trends within species (e.g., two apparently increasing populations and two apparently decreasing populations in Tanzania), so we would want to think a lot more about how we scale up from population to species to capture this variation in a way that is most useful to our question. Here we might realize we want to try to understand the drivers of this variability -- for this we would need to return to Step 1 and reformulate our question, then go through the workflow again, likely adding in potential predictors such as some metric of conservation efforts for that species and or population (e.g., stopping the Ivory trade for elephants, metrics of poaching in different parks) and possibly also country, which may capture differences in efforts related to conservation that are not yet measured. We would do this, returning to Steps 2-5 likely several times until we have a model that is good enough for our purposes here. And then we would likely add in more countries and species.

Alternatively, we may feel this model is good enough for our question and think about how to robustly add in the species and geography we originally excluded. 
