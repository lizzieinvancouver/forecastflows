...
What is trend in mammal species population numbers in Africa, both overall and how does it vary acrosss species and populations? 
**Note**: we will actually focus on African mammals with 10+ populations with 5+ observations per population.
...
nspecies <- 9
npops_persp <- round(runif(nspecies, 1, 3)) # no. of different populations per species
npops <- sum(npops_persp) # total no. of different populations
nobs_perpop <- round(runif(npops, 5, 15)) # no. of different observations per population
...
for(np in nobs_perpop){
  years_p <- sample(1960:2020, size = np, replace = FALSE)
  years <- c(years, years_p)
}


# nested intercepts
mu_species_alpha <- rnorm(n = nspecies, mean = 200, sd = 25)
sig_species_alpha <- rnorm(n = nspecies, mean = 5, sd = 2)
...
# nested slopes
mu_species_beta <- rnorm(n = nspecies, mean = 0, sd = 0.1)
sig_species_beta <- rnorm(n = nspecies, mean =0, sd = 0.5)
mu_pop_beta <- rnorm(n = npops, mean = mu_species_beta[spid_perpop], sd = 0.2)
sig_pop_beta <- abs(rnorm(n = npops))
beta <- rnorm(n = npops, mean = mu_pop_beta, sd = sig_pop_beta)

sigma_obs <- abs(rnorm(1, mean = 10, sd = 1))
...
for(p in 1:npops){
  ...
  # yhat_p <- alpha_p
  y_p <- rnorm(n = nobs_p, mean = yhat_p, sd = sigma_obs)
  y <- c(y, y_p)
}
...
dataplot <- data.frame(
  ...
  population = as.character(popid_perobs)
)
...
nestlm <- lm(y ~ year * species/population, data = dataplot)
...
species_slope <- c(coefs["year", "Estimate"], sapply(2:nspecies, function(sp){coefs["year", "Estimate"] + coefs[paste0("year:species", sp), "Estimate"]}))
species_slope_sd <- c(coefs["year", "Std. Error"], sapply(2:nspecies, function(sp){coefs[paste0("year:species", sp), "Std. Error"]}))
...
```
*Add some text here showing the model output returning parameters.* 
...
nestlm <- lm(value ~ year * genus_species/ID, data = lpi_subset_rsh)
coefs <- summary(nestlm)$coefficients
...
slopes <- data.frame(est=species_slope, est.sd=species_slope_sd, species)
...
retrodata <- data.frame(obs = lpi_subset_rsh$value, pred = predict(nestlm),
