...
% This appears to be only increasing in newer models that generally have more complexity (and an ever-increasing number of parameters), making it more difficult to increase scientific understanding \citep{Franklin2020}, and suggest or model potential policies.
...
% Trend estimation still relies on a one-way `inference',  and most of the time is spent fitting the model to empirical data (Fig. 1).
...
% should we keep the sentence below % %emw6Apr: I suggest we delete. 
...
% There is thus room for improvment, with a workflow that addresses these flaws while taking account the reality of working with ecological data.
...
%emw6Apr: I like the shift to 'parameter estimation' instead of trend estimation, but I wonder if we could make that shift clearer above. Maybe somewhere we say that the workflow would  encourage a focus on the full model and its parameters: For trend estimates, the trend becomes just one parameter that must be carefully interpreted alongside others such as the  global mean or intercept and the estimate variance. 
